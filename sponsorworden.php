<!DOCTYPE html>
<html lang="nl">
<?php require 'header.php' ?>
<?php require 'menu.php' ?>

<div class="container">
    <div class="main-content">
        <div class="row">
            <div class="col-12 col-lg-8 col-md-9 col-sm-12">
                <h2>Sponsor worden?</h2>
                <p>Hieronder een overzicht van de mogelijke sponsorpakketten. Wil je je aanmelden of heb je een vraag?
                    Stuur ons een <a href="mailto:triathlonlangedijk@hotmail.com?Subject=Sponsoring">mail</a>.
                </p>
                <ol>
                    <table>
                        <td>
                            <li>Hoofdsponsor &#40;exclusief&#41;</li>
                        </td>
                        <td class="align-right">&euro; 1000,&ndash;</td>
                        </tr>
                        <tr>
                            <td>&#40;jaarlijks voor de termijn van 3 jaar&#41;</td>
                        </tr>
                    </table>
                    <br>
                    Triathlon Langedijk stelt hiervoor ter beschikking&#59;
                    <ul>
                        <li>Logo op de voorpagina programmaboekje</li>
                        <li>Mogelijkheid tot plaatsing van &#40;eigen&#41; sponsorbord bij de start en finish &#40;ca
                            100cm hoog en 300cm breed&#41;
                        </li>
                        <li>Mogelijkheid tot plaatsing van &#40;eigen&#41; sponsorbord in het Parc Ferm&eacute; &#40;ca
                            100cm hoog en 300cm breed&#41;
                        </li>
                        <li>Naamsvermelding door omroeper</li>
                        <li>Naamsvermelding in alle communicatie-uitingen &#40;poster&#47;advertentie&#47;social
                            media&#47;persberichten&#47;berichten deelnemers&#41;
                        </li>
                        <li>Logo plus link op de website</li>
                        <li>Mogelijkheid tot stoppen van flyer&#47;give-away in deelnemerstasje</li>
                        <li>20 consumptiebonnen voor een drankje na afloop in De Buizerd</li>
                        <li>Gratis deelname aan de triathlon voor individuele wedstrijd &#40;2x&#41; en triowedstrijd
                            &#40;4x&#41;
                        </li>
                    </ul>
                    <br>
                    <hr>
                    <br>
                    <table>
                        <td>
                            <li>Naamsponsor pilot Team Relay &#40;exclusief&#41;</li>
                        </td>
                        <td class="align-right">&euro; 500,&ndash;</td>
                        </tr></table>
                    <br>
                    Triathlon Langedijk stelt hiervoor ter beschikking&#59;
                    <ul>
                        <li>Naamgeving aan de pilot Team Relay, de sprintestafette die voorafgaand aan de individuele
                            wedstrijd plaatsvindt
                        </li>
                        <li>Mogelijkheid tot plaatsing van &#40;eigen&#41; sponsorbord in het Parc Ferm&eacute; &#40;ca
                            100cm hoog en 300cm breed&#41;
                        </li>
                        <li>Naamsvermelding door omroeper</li>
                        <li>Naamsvermelding in alle communicatie-uitingen &#40;poster&#47;advertentie&#47;social
                            media&#47;persberichten&#47;berichten deelnemers&#41;
                        </li>
                        <li>Logo plus link op de website</li>
                        <li>Mogelijkheid tot stoppen van flyer&#47;give-away in deelnemerstasje</li>
                        <li>10 consumptiebonnen voor een drankje na afloop in De Buizerd</li>
                        <li>Gratis deelname aan de Team Relay &#40;2x&#41;</li>
                        <li>Gratis deelname aan de triathlon voor individuele wedstrijd &#40;1x&#41; en triowedstrijd
                            &#40;1x&#41;
                        </li>
                    </ul>
                    <br>
                    <hr>
                    <br>
                    <table>
                        <td>
                            <li>Sponsor deelnemerstassen &#40;exclusief&#41;</li>
                        </td>
                        <td class="align-right">&euro; 500,&ndash;</td>
                        </tr></table>
                    <br>
                    Triathlon Langedijk stelt hiervoor ter beschikking&#59;
                    <ul>
                        <li>Alle deelnemers krijgen bij aanmelding een tas (plastic rugtasje) met hierin hun
                            wedstrijdnummer, chip, wedstrijdinformatie en een aantal geschenken van sponsoren. Op de tas
                            kan een bedrijfslogo worden gedrukt. In totaal zullen er 250 tassen worden bedrukt
                        </li>
                        <li>Logo in het programmaboekje</li>
                        <li>Naamsvermelding in alle communicatie-uitingen &#40;poster&#47;advertentie&#47;social
                            media&#47;persberichten&#47;berichten deelnemers&#41;
                        </li>
                        <li>Logo plus link op de website</li>
                        <li>Mogelijkheid tot stoppen van flyer&#47;give-away in deelnemerstasje</li>
                        <li>10 consumptiebonnen voor een drankje na afloop in De Buizerd</li>
                        <li>Gratis deelname aan de triathlon voor individuele wedstrijd &#40;1x&#41; en triowedstrijd
                            &#40;1x&#41;
                        </li>
                    </ul>
                    <br>
                    <hr>
                    <br>
                    <table>
                        <td>
                            <li>Baniersponsor</li>
                        </td>
                        <td class="align-right">&euro; 250,&ndash;</td>
                        </tr></table>
                    <br>
                    Triathlon Langedijk stelt hiervoor ter beschikking&#59;
                    <ul>
                        <li>Mogelijkheid tot plaatsing van &#40;eigen&#41; banier in het Parc Ferm&eacute; en&#47;of
                            langs de route
                        </li>
                        <li>Mogelijkheid tot plaatsing van &#40;eigen&#41; sponsorbord in het Parc Ferm&eacute; &#40;ca
                            100cm hoog en 300cm breed&#41;
                        </li>
                        <li>Naamvermelding door omroeper</li>
                        <li>Logo in het programmaboekje</li>
                        <li>Naamsvermelding in alle communicatie-uitingen &#40;poster&#47;advertentie&#47;social
                            media&#47;persberichten&#47;berichten deelnemers&#41;
                        </li>
                        <li>Logo plus link op de website</li>
                        <li>Mogelijkheid tot stoppen van flyer&#47;give-away in deelnemerstasje</li>
                        <li>10 consumptiebonnen voor een drankje na afloop in De Buizerd</li>
                        <li>Gratis deelname aan de triathlon voor individuele wedstrijd &#40;1x&#41; en triowedstrijd
                            &#40;1x&#41;
                        </li>
                    </ul>
                    <br>
                    <hr>
                    <br>
                    <table>
                        <td>
                            <li>Grote vriend van Triathlon Langedijk</li>
                        </td>
                        <td class="align-right">&euro; 100,&ndash;</td>
                        </tr></table>
                    <br>
                    Triathlon Langedijk stelt hiervoor ter beschikking&#59;
                    <ul>
                        <li>Logo in het programmaboekje</li>
                        <li>Naamsvermelding in alle communicatie-uitingen &#40;poster&#47;advertentie&#47;social
                            media&#47;persberichten&#47;berichten deelnemers&#41;
                        </li>
                        <li>Logo plus link op de website</li>
                        <li>Mogelijkheid tot stoppen van flyer&#47;give-away in deelnemerstasje</li>
                        <li>10 consumptiebonnen voor een drankje na afloop in De Buizerd</li>
                        <li>Gratis deelname aan de triathlon voor triowedstrijd &#40;1x&#41;</li>
                    </ul>
                    <br>
                    <hr>
                    <br>
                    <table>
                        <td>
                            <li>Kleinere vriend van Triathlon Langedijk</li>
                        </td>
                        <td class="align-right">&euro; 50,&ndash;</td>
                        </tr></table>
                    <br>
                    Triathlon Langedijk stelt hiervoor ter beschikking&#59;
                    <ul>
                        <li>Logo in het programmaboekje</li>
                        <li>Naamsvermelding in alle communicatie-uitingen &#40;poster&#47;advertentie&#47;social
                            media&#47;persberichten&#47;berichten deelnemers&#41;
                        </li>
                        <li>Logo plus link op de website</li>
                    </ul>
                    <br>
                    <hr>
                    <br>
                    <td>
                        <li>Divers</li>
                    </td>
                    <br>
                    <p>Wil je Triathlon Langedijk op een andere manier sponsoren? Dan is sponsoring op maat ook
                        mogelijk. Uiteraard staat hier een passende beloning tegenover, in goed overleg. Help ons
                        bijvoorbeeld aan de volgende materialen&#59;</p>
                    <ul>
                        <li>Douchewagen</li>
                        <li>Badmutsen &#40;250x, kan uiteraard met logo, kosten aanschaf en bedrukking voor
                            sponsor&#41;
                        </li>
                        <li>Hekken</li>
                        <li>Een filmer die ons evenement filmt en er een &#40;promotie&#41;video van maakt</li>
                        <li>Startnummers &#40;250x, kan uiteraard met logo, kosten aanschaf en bedrukking voor
                            sponsor&#41;
                        </li>
                        <li>Start&ndash;finishboog</li>
                        <li>Kleding voor de vrijwilligers &#40;100 t&ndash;shirts&#41;</li>

                    </ul>
                </ol>
            </div>
            <?php require 'sidebar.php' ?>
        </div>
    </div>

    <?php require 'footer.php' ?>
</html>